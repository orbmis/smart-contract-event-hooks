...
Both the publisher and subscriber contracts **MUST** register in a specific register contract, similarly to how smart contracts register an interface in the [ERC-1820](https://eips.ethereum.org/EIPS/eip-1820.md) contract.  The registry contract **MUST** must use a deterministic deployment mechanism, i.e. using a factory contract and a specific salt.
...
For this reason, it is recommended to use [ERC-721](https://eips.ethereum.org/EIPS/eip-712.md) Typed Data Signatures.  In this case the process that initiates the hook should create the signature according to the following data structure:
